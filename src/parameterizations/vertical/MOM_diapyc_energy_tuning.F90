...
type, public :: diapyc_energy_tuning_CS ; private
  ...
  !! Kd_add is added. The values specify the density at which the extra diffusivity starts to increase
  !! from 0, hits its full value, starts to decrease again, and is back to 0 [R ~> kg m-3].
  real, dimension(4)  :: rho_range !! 4 values that define density range where Kd is enhanced [kg m-3].
  !< Optional parameters that define the latitude and longitude ranges over which a diffusivity is added.
  real, dimension(4)  :: lat_range !! 4 values that define latitude range where Kd is enhanced [degrees_N].
  real, dimension(4)  :: lon_range !! 4 values that define longitude range where Kd is enhanced [degrees_E].
...
# include "version_variable.h"
  ...
  CS%id_EnKdTuned = register_diag_field('ocean_model', 'energy_Kd_tuned', diag%axesT1, Time, &
                 "Column-integrated rate of energy consumption by diapycnal mixing after tuning (base + added Kd).",&
                  units="W m-2", conversion=US%RZ3_T3_to_W_m2*US%L_to_Z**2)
  CS%id_EnChangeTuned = register_scalar_field('ocean_model', 'PE_chg_Kd_tuned', Time, diag, &
          'Globally integrated change in energy due to diapycnal mixing.', units='W')
          !, conversion=US%RZ3_T3_to_W_m2*US%L_to_Z**2) ! unsure of conversion factor for PE_chg_Kd_tuned
